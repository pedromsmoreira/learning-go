...
func main() {
	...
	strB, _ := json.Marshal("gopherino")
	fmt.Println(string(strB))

	// And here are some for slices and maps, which encode to JSON arrays and objects as you’d expect.
	slcC := []string{"apple", "orange", "avocado"}
	slcB, _ := json.Marshal(slcC)
	fmt.Println(string(slcB))

	mapD := map[string]int{"apple": 5, "lettuce": 10}
	...
	res1D := &response1{
		Page:   1,
		Fruits: []string{"apple", "orange", "avocado"},
	}
	...
	res2D := &response2{
		Page:   1,
		Fruits: []string{"apple2", "orange2", "avocado2"},
	}
